...
import microc.frontend.ast.Loc
import microc.frontend.util.parsing.combinator.{CharInput, StringParsers}
...
/**
  ...
  *
  * The [[microc.frontend.parser.Parser]] can be mixed in directly as the parser is stateless.
  */
class PCParser extends StringParsers with microc.frontend.parser.Parser {
  ...
  lazy val ArrayAccess: Parser[ast.Expr => ast.ArrayAccess] =
    (C <~ "[") ~ (Expr <~ "]") ^^ {
      case loc ~ index =>
        expr =>
          ast.ArrayAccess(expr, index, loc)
    } label "ArrayAccess"
